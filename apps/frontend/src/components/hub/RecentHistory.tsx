  ...
  return (
    <div className="h-full flex flex-col">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="glass rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 relative overflow-hidden group flex-1 flex flex-col backdrop-blur-3xl border-white/5"
      >
        <h3 className="text-[10px] sm:text-xs font-mono uppercase text-zinc-500 tracking-[0.2em] sm:tracking-[0.3em] mb-6 sm:mb-8 md:mb-10 flex items-center gap-2 sm:gap-3 relative z-10">
          <History className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-500 shrink-0" />
          Recent History
        </h3>
        ...
        <div className="flex-1 overflow-y-auto pr-1 sm:pr-2 space-y-2 sm:space-y-3 scrollbar-hide relative z-10 min-h-[240px] sm:min-h-[300px]">
          {recentMatches.length > 0 ? (
            recentMatches.map((match) => (
              <div
                key={match.id}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-0 p-4 sm:p-6 bg-white/[0.02] border border-white/5 rounded-xl sm:rounded-2xl hover:bg-white/[0.04] transition-all cursor-pointer group/row"
              ...
              >
                ...
                <div className="flex items-center gap-3 sm:gap-6 sm:w-48 shrink-0 order-1 sm:order-1">
                  <div className="font-mono text-[10px] sm:text-xs text-zinc-600 uppercase tracking-[0.15em] sm:tracking-[0.2em] tabular-nums">
                    {new Date(match.created_at || '').toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
                  </div>
                  <span className="font-mono text-[9px] sm:text-[10px] text-zinc-700 uppercase tracking-tight">#{match.id.substring(0, 8)}</span>
                </div>
                ...
                <div className="flex-1 flex flex-row items-center justify-center gap-3 sm:gap-8 md:gap-12 px-2 sm:px-4 md:px-8 order-2 sm:order-2 min-w-0">
                  ...
                  <div className="flex flex-col items-center gap-1 shrink-0">
                    <span className="text-[10px] sm:text-xs font-black italic text-zinc-800 tracking-tighter uppercase grayscale">VS</span>
                    {match.tx_hash && !match.tx_hash.startsWith('{') && (
                      <div className="px-1.5 sm:px-2 py-0.5 bg-[#00FF00]/10 rounded-full border border-[#00FF00]/20 flex items-center gap-1 sm:gap-1.5">
                        ...
                        <span className="text-[9px] sm:text-[10px] font-mono text-[#00FF00] uppercase font-bold tracking-[0.05em]">Secured</span>
                      </div>
                    )}
                  </div>
                ...
                <div className="w-full sm:w-32 flex justify-end shrink-0 order-3 sm:order-3">
                  <div className="flex items-center gap-1.5 sm:gap-2 group-hover/row:text-primary transition-colors">
                    <span className="font-mono text-[9px] sm:text-[10px] text-zinc-600 uppercase tracking-widest font-bold hidden sm:inline">Details</span>
                    <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-zinc-800 shrink-0" />
                  </div>
                </div>
              </div>
            ))
          ) : (
            <div className="flex flex-col items-center justify-center h-full min-h-[240px] sm:min-h-[300px] border border-dashed border-white/10 rounded-xl sm:rounded-2xl bg-white/[0.02] p-4">
              <p className="font-mono text-[10px] sm:text-xs text-zinc-700 uppercase tracking-[0.2em] sm:tracking-[0.3em] text-center">No records indexed.</p>
            </div>
          )}
        </div>
      </motion.div>
