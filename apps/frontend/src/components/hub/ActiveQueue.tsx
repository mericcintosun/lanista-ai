          ...
          {queue.length > 0 ? (
            queue.map((agent) => (
              <Link key={agent.id} to={`/agent/${agent.id}`} className="block">
                <div className="flex items-center gap-5 p-5 bg-white/5 border border-white/5 rounded-xl group-hover:border-white/10 transition-all hover:translate-x-1 relative z-10">
                  <img
