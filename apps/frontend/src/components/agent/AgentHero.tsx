...
export function AgentHero({ agent, history }: AgentHeroProps) {
  ...
  return (
    <div className="space-y-6">
      {/* ── TOP HERO CARD ── */}
      <div className="glass bg-zinc-900/40 p-6 sm:p-8 rounded-[2rem] relative overflow-hidden">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 relative z-10">
          {/* Avatar */}
          <div className="shrink-0 relative">
            <div className="absolute -inset-2 bg-primary/20 blur-2xl rounded-full opacity-50" />
            <img
              src={agent.avatar_url || `https://api.dicebear.com/7.x/bottts/svg?seed=${agent.name}`}
              alt={agent.name}
              className="w-32 h-32 rounded-3xl object-cover border-2 border-white/5 shadow-2xl relative"
            />
          </div>
          ...
          <div className="flex-1 space-y-6 text-center md:text-left">
            <div>
              ...
              {agent.description && (
                <p className="text-zinc-500 text-sm max-w-xl italic mt-3 border-l-2 border-white/5 pl-4">
                  "{agent.description}"
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
              <div className="flex items-center bg-black/40 border border-white/5 rounded-lg px-3 py-1.5 gap-2">
              ...
              {agent.created_at && (
                <div className="bg-black/20 px-3 py-1.5 rounded-lg border border-white/5 font-mono text-xs text-zinc-500 uppercase tracking-widest">
      ...
      {/* ── STATS ROW ── */}
      <div className="grid grid-cols-2 md:grid-cols-4 glass bg-zinc-900/40 rounded-2xl overflow-hidden">
        <div className="p-6 border-r border-white/5 flex flex-col items-center text-center group hover:bg-white/[0.02] transition-colors">
          <Trophy className="w-4 h-4 text-zinc-500 mb-3 group-hover:text-primary transition-colors" />
        ...
        <div className="p-6 border-r border-white/5 flex flex-col items-center text-center group hover:bg-white/[0.02] transition-colors">
          <Swords className="w-4 h-4 text-zinc-500 mb-3 group-hover:text-primary transition-colors" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6">
        {/* Passport Card */}
        <div className="md:col-span-8 glass bg-zinc-900/60 p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl md:rounded-[2rem] border-cyan-500/20 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-48 sm:w-64 h-48 sm:h-64 bg-cyan-500/5 blur-[100px] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 relative z-10">
            <div className="flex items-center gap-3 sm:gap-5">
              <div className="w-12 h-12 sm:w-14 md:w-16 sm:h-14 md:h-16 rounded-xl sm:rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center shrink-0">
                <Shield className="w-6 h-6 sm:w-7 md:w-8 sm:h-7 md:h-8 text-cyan-400" />
              </div>
              <div className="min-w-0">
                ...
                <p className="text-[10px] sm:text-xs font-mono text-cyan-400/70 uppercase tracking-widest mt-0.5 sm:mt-1">Lany Identity • Immutable On-Chain Record</p>
                <div className="mt-1.5 sm:mt-2 text-[9px] sm:text-[10px] font-mono bg-cyan-500/10 text-cyan-400/80 border border-cyan-500/20 px-2 py-0.5 rounded-sm inline-block uppercase tracking-widest">ERC #8004 • Verified</div>
              </div>
            </div>
            <div className="flex items-center gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 bg-black/40 border border-white/5 rounded-lg sm:rounded-xl shrink-0 self-start sm:self-auto">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 relative z-10">
            ...
            <div className="bg-black/40 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-white/5">
              ...
              <span className="text-lg sm:text-xl md:text-2xl font-black text-cyan-400 italic">{passport?.reputationScore || '000'}</span>
            </div>
            <div className="bg-black/40 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-white/5">
        ...
        {/* Level/Tier Section */}
        <div className="md:col-span-4 glass bg-zinc-900/40 p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl md:rounded-[2rem] flex flex-col justify-center items-center text-center">
