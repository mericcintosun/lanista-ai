    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {monitors.map(({ icon: Icon, label, value, dot }) => (
        <div key={label} className="glass p-8 relative overflow-hidden flex flex-col items-center text-center rounded-2xl group transition-all duration-300 hover:border-primary/30">
          ...
          <div className="text-xs sm:text-sm text-zinc-500 font-mono uppercase tracking-[0.3em] mb-2 font-bold">{label}</div>
          <div className="text-sm sm:text-base font-black text-white uppercase italic tracking-wider">{value}</div>
        </div>
      ))}
    </div>
  );
