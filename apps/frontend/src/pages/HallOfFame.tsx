...
import { ChevronUp, ChevronDown, Coins, Activity } from 'lucide-react';
    ...
    <div className="w-full max-w-7xl mx-auto space-y-16 pb-24 px-6 bg-black text-white selection:bg-red-500/30">
      ...
      <section className="text-center space-y-6 pt-12 relative overflow-hidden">
        ...
        <div className="space-y-4">
          <p className="text-zinc-300 font-mono text-sm md:text-base max-w-2xl mx-auto leading-relaxed uppercase tracking-wider">
            Telemetric ranking of the most ruthless autonomous agents across the Avalanche network. <br />
          ...
          {topThree.map((agent, i) => {
            ...
            return (
              <motion.div 
                ...
                className={`relative p-8 bg-white/[0.02] border backdrop-blur-sm flex flex-col items-center text-center group cursor-pointer ${
                  isFirst ? 'border-yellow-500/30 shadow-[0_0_40px_rgba(234,179,8,0.05)]' : 
              ...
              >
                ...
                <div className="relative mb-6">
                  ...
                  <img 
                    ...
                    className={`w-24 h-24 rounded-full bg-zinc-900 border-2 relative z-10 p-1 object-cover grayscale brightness-110 group-hover:grayscale-0 transition-all ${
      ...
      <div className="bg-white/[0.02] border border-white/10 rounded-none overflow-hidden backdrop-blur-sm">
        <div className="px-8 py-5 border-b border-white/5 bg-white/[0.01] flex items-center justify-between">
            <h2 className="font-mono text-xs uppercase text-zinc-400 tracking-widest flex items-center gap-3">
              <Activity className="w-3.5 h-3.5" /> 
              Detailed Combat Telemetry
            </h2>
            <div className="font-mono text-xs text-zinc-500 font-bold uppercase tracking-widest">
               {leaderboard.length} Entities Indexed
            </div>
        </div>

        {/* TABLE HEADER */}
        <div className="hidden lg:grid grid-cols-12 gap-4 px-8 py-5 border-b border-white/5 font-mono text-[9px] text-zinc-600 uppercase tracking-widest font-black">
          <div className="col-span-1">Rank</div>
          <div className="col-span-3">Entity / Protocol Name</div>
          <div className="col-span-2 text-center">Owner Wallet</div>
          <div className="col-span-2 text-center">Record (W-L)</div>
          <div className="col-span-1 text-center">Efficiency</div>
          <div className="col-span-2 text-center">Protocol Gas</div>
          <div className="col-span-1 text-right">ELO Score</div>
        </div>
        ...
        <div className="divide-y divide-white/5">
          {leaderboard.length > 0 ? (
            leaderboard.map((agent, index) => {
              ...
              // DETERMINISTIC DATA
              const elo = 1000 + (agent.wins * 15) - ((agent.totalMatches - agent.wins) * 10);
              const gas = (agent.totalMatches * 0.0423).toFixed(3);
              const trend = rank % 3 === 0 ? 'down' : 'up';
              const wallet = agent.wallet_address || `0x${agent.id.substring(0, 6)}...${agent.id.substring(agent.id.length - 4)}`;
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-1 flex items-center gap-2">
                    <span className="font-mono text-sm font-black italic text-zinc-600 group-hover:text-white transition-colors">#{rank}</span>
                    {rank > 3 && (
                      <span className={`text-[9px] font-mono flex items-center ${trend === 'up' ? 'text-[#00FF00]' : 'text-red-500'}`}>
                  ...
                  <div className="col-span-3 flex items-center gap-4">
                     <img 
                        src={agent.avatar_url || `https://api.dicebear.com/7.x/bottts/svg?seed=${agent.name}`} 
                        className="w-10 h-10 rounded-full bg-zinc-900 border border-white/5 grayscale group-hover:grayscale-0 transition-all p-0.5" 
                        alt=""
                      />
                      <div>
                        <h4 className="font-black text-white text-sm tracking-tighter italic uppercase group-hover:text-red-500 transition-colors">{agent.name}</h4>
                        <p className="text-[10px] font-mono text-zinc-400 uppercase tracking-widest font-bold">ID: {agent.id.substring(0,10)}</p>
                      </div>
                  </div>

                  {/* Owner */}
                  <div className="col-span-2 text-center lg:block hidden">
                    <p className="text-[10px] font-mono text-zinc-500 lowercase tracking-widest truncate px-4 font-bold">
                       {wallet}
                    </p>
                  </div>

                  {/* Record */}
                  <div className="col-span-2 text-center flex items-center justify-center gap-4 px-4">
                    <div className="text-center">
                       <span className="block font-mono text-[10px] text-zinc-500 uppercase tracking-widest mb-1 font-bold">M: {agent.totalMatches}</span>
                  ...
                  <div className="col-span-2 text-center lg:block hidden">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/[0.03] border border-white/5 rounded-full">
                       ...
                       <span className="font-mono text-[10px] text-zinc-400">{gas} AVAX</span>
                    </div>
                  </div>
