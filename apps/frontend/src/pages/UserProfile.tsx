...
import { Activity, Trophy, Swords, Zap, Copy, Check, Globe, Shield, Sparkles, LogOut, Layout } from 'lucide-react';
...
export default function UserProfile() {
  ...
  const renderClaimForm = () => (
    <div className="space-y-2">
      {(bindError || bindSuccess) && (
        <div className={`p-2 border rounded font-mono text-[8px] uppercase ${
      ...
      {!bindCode && !bindSuccess && (
        <div className="space-y-2">
          <input 
            type="text" 
            placeholder="Lany ID" 
            className="w-full bg-black/40 border border-white/5 rounded px-2.5 py-2 text-white font-mono text-[9px] uppercase focus:border-primary/50 outline-none placeholder:text-zinc-600"
          ...
          <div className="flex gap-1.5">
            <input 
              type="password" 
              placeholder="API Key" 
              className="flex-1 bg-black/40 border border-white/5 rounded px-2.5 py-2 text-white font-mono text-[9px] uppercase focus:border-primary/50 outline-none placeholder:text-zinc-600"
              value={bindApiKey}
            ...
            <button 
              ...
              className="px-3 py-2 bg-primary text-white rounded font-black uppercase text-[8px] tracking-wider hover:bg-primary/90 disabled:opacity-30 active:scale-95 transition-all shrink-0"
            >
      ...
      {bindCode && !bindSuccess && (
        <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          <div className="flex items-center gap-1.5">
            <span className="flex-1 font-mono text-[8px] text-zinc-400 p-2 bg-black/40 rounded border border-white/5 select-all truncate">{bindCode}</span>
            <button onClick={() => { navigator.clipboard.writeText(bindCode!); toast.success('Copied!'); }} className="p-2 bg-white/5 border border-white/10 rounded text-white hover:border-white/20 shrink-0">
          ...
          <button 
            ...
            className="w-full py-2 bg-white text-black text-[9px] font-black rounded uppercase tracking-wider hover:bg-zinc-200 transition-all"
          >
            Post on X
          </button>
          <div className="flex gap-1.5">
            <input 
              type="text" 
              placeholder="@handle" 
              className="w-20 bg-black/40 border border-white/5 rounded px-2 py-1.5 text-white font-mono text-[9px] focus:border-green-500/50 outline-none placeholder:text-zinc-600"
            ...
            <input 
              ...
              className="flex-1 bg-black/40 border border-white/5 rounded px-2 py-1.5 text-white font-mono text-[9px] focus:border-green-500/50 outline-none placeholder:text-zinc-600"
              value={bindTweetUrl}
          ...
          <button 
            ...
            disabled={bindLoading}
            className="w-full py-2 bg-secondary text-white rounded font-black uppercase text-[9px] tracking-wider hover:bg-secondary/90 disabled:opacity-30 transition-all"
          >
  ...
  return (
    <div className="pb-24 space-y-10 max-w-6xl mx-auto px-4 sm:px-6 relative pt-0">
        ...
        <PageHeader 
          ...
          actions={
            <button 
              ...
              className="px-6 py-2.5 bg-primary/10 border border-primary/20 text-primary/60 font-black rounded-lg hover:bg-primary hover:text-white transition-all text-[10px] uppercase tracking-widest flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          <Reveal key={i} delay={i * 0.05}>
            <div className="bg-zinc-900/40 border border-white/5 rounded-xl p-4 backdrop-blur-xl group hover:border-white/10 transition-all min-h-[88px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 items-stretch lg:min-h-[280px]">
        {/* DOSSIER */}
        <Reveal direction="left" delay={0.2} className="min-h-0">
          <div className="h-full bg-zinc-900/40 border border-white/5 rounded-2xl p-5 backdrop-blur-3xl relative overflow-hidden flex flex-col">
               <div className="relative z-10 flex flex-col flex-1 min-h-0">
                  <div className="flex items-center gap-3">
                    <img src={user.user_metadata?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.email}`} className="w-12 h-12 rounded-xl border border-white/10 bg-black p-0.5 shrink-0" />
                    <div className="min-w-0 flex-1">
                      <h3 className="text-base font-black text-white italic uppercase tracking-tighter truncate">{profileData?.callsign || 'COMMANDER'}</h3>
                      <p className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest truncate">{user.email}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 pt-4 mt-4 border-t border-white/5 flex-1">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="font-mono text-[9px] text-zinc-400 uppercase truncate">{profileData?.sector || 'Unknown'}</span>
                    </div>
                    {profileData?.bio && (
                      <p className="font-mono text-[9px] text-zinc-500 leading-relaxed line-clamp-2">{profileData.bio}</p>
                    )}
                  </div>

                  <div className="pt-4 mt-auto">
                    <div className="flex items-center gap-1.5">
                      <span className="flex-1 font-mono text-[8px] text-zinc-600 p-2 bg-black/40 rounded-lg border border-white/5 truncate">{user.id}</span>
                      <button 
                        onClick={() => {
        ...
        {/* LINK LANY */}
        <Reveal direction="up" delay={0.3} className="min-h-0">
          <div className="h-full flex flex-col glass bg-zinc-900/60 border border-primary/20 rounded-2xl p-5 relative overflow-hidden">
            <div className="flex items-center justify-between gap-2 mb-3 shrink-0">
               <h3 className="text-sm font-black text-white italic uppercase tracking-tighter flex items-center gap-1.5">
                  <XIcon className="w-4 h-4 fill-primary" /> Link Lany
               </h3>
               <div className="flex items-center gap-1.5 bg-black/40 border border-white/10 px-2 py-1 rounded-full">
                  <div className="w-1 h-1 rounded-full bg-green-500 animate-pulse" />
                  <span className="text-[8px] font-mono text-zinc-400 uppercase">Active</span>
               </div>
            </div>
        ...
        {/* MY LANYS */}
        <Reveal direction="right" delay={0.4} className="min-h-0">
          <div className="h-full flex flex-col bg-zinc-900/40 border border-white/5 rounded-2xl p-5 backdrop-blur-xl overflow-hidden">
            <div className="flex items-center justify-between mb-3 shrink-0">
               <h3 className="text-sm font-black italic text-white uppercase tracking-tighter flex items-center gap-1.5">
                  <Layout className="w-4 h-4 text-primary" /> My Lanys
               </h3>
               {activeAgents > 0 && <span className="font-mono text-[9px] text-zinc-500 uppercase">{activeAgents} active</span>}
            </div>
            ...
            <div className="flex-1 min-h-0 grid grid-cols-1 gap-2 overflow-auto">
                  {activeAgents > 0 ? (
                    ...
                    <div className="border border-dashed border-white/5 rounded-xl p-6 text-center flex flex-col items-center justify-center space-y-2 min-h-[100px]">
                       ...
                       <p className="font-mono text-[9px] text-zinc-600 uppercase tracking-wider">Link a Lany to get started</p>
                    </div>
                  )}
                </div>
          </div>
